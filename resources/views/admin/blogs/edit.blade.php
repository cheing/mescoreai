...
<div class="row">
  <div class="col-lg-12 col-sm-12 grid-margin">
    <form method="POST" id="form-blog" action="{{ url('admin/blogs/' . $blog->id) }}" enctype="multipart/form-data"
      class="forms-sample">
      @csrf
      @method('PUT')
      ...
      <div class="card">
        <div class="card-header header-sm">
          <div class="d-flex">
            <h2 class="card-title mb4">Edit Blog</h2>
            <div class="ml-auto">
        ...
        <div class="card-body">
            ...
            <div class="tab-pane fade {{ $index === 0 ? 'show active' : '' }}" id="lang-{{ $lang['code'] }}"
              ...
              <div class="form-group">
                ...
                <input type="text" name="title_{{ $lang['code'] }}" class="form-control" required
                  value="{{ $blog->translations->where('locale', $lang['code'])->first()->title ?? '' }}">
              </div>
              <div class="form-group">
                <label>Excerpt </label>
                <textarea name="short_description_{{ $lang['code'] }}" class="form-control"
                  rows="3">{{ $blog->translations->where('locale', $lang['code'])->first()->short_description ?? '' }}</textarea>
              </div>
              <div class="form-group">
                <label>Content</label>
                <textarea name="content_{{ $lang['code'] }}"
                  class="form-control summernote">{{ $blog->translations->where('locale', $lang['code'])->first()->content ?? '' }}</textarea>
              </div>
              ...
              <div class="form-group">
                ...
                <input type="text" name="meta_title_{{ $lang['code'] }}" class="form-control" maxlength="255"
                  value="{{ $blog->translations->where('locale', $lang['code'])->first()->meta_title ?? '' }}">
              </div>
              <div class="form-group">
                <label>Meta Keywords</label>
                <input type="text" name="meta_keywords_{{ $lang['code'] }}" class="form-control" maxlength="255"
                  value="{{ $blog->translations->where('locale', $lang['code'])->first()->meta_keywords ?? '' }}">
              </div>
              <div class="form-group">
                <label>Meta Description</label>
                <textarea name="meta_description_{{ $lang['code'] }}" class="form-control" rows="3"
                  maxlength="500">{{ $blog->translations->where('locale', $lang['code'])->first()->meta_description ?? '' }}</textarea>
              </div>
          ...
          <div class="form-group row">
            ...
            <div class="col-md-10 d-flex align-items-center">
              <div class="user-avatar mb-auto">
                <img src="{{ $blog->thumbnail ? asset('storage/'.$blog->thumbnail) : asset('images/no_image.jpg') }}"
                  alt="Blog Image" class="profile-img img-lg rounded-circle" id="thumbnail_url" />
                <span class="edit-avatar-icon" data-toggle="modal" data-target="#modalImage">
                  <i class="mdi mdi-upload"></i>
                </span>
              </div>
              <input type="hidden" name="thumbnail" value="{{ $blog->thumbnail }}" id="input-thumbnail" />
            </div>
          </div>
          ...
          <div class="form-group row">
            ...
            <div class="col-md-10">
              <select name="status" class="form-control">
                ...
                <option value="{{ $k }}" {{ $blog->status == $k ? 'selected' : '' }}>
                  {{ $v }}
                </option>
                @endforeach
              </select>
          ...
          <div class="form-group row">
            ...
            <div class="col-md-10">
              <select name="blog_category_id[]" class="js-example-basic-multiple" multiple>
                @foreach($categories as $cat)
                <option value="{{ $cat->id }}" {{ in_array($cat->id, $blog->categories->pluck('id')->toArray() ?? [])
                  ? 'selected' : '' }}>
...
@section('footer')
{!! JsValidator::formRequest('App\Http\Requests\UpdateBlogRequest', '#form-blog'); !!}
...
<script>
  ...
  $.ajax({
    ...
    notifySuccess('Blog successfully updated.');
    setTimeout(function() {
